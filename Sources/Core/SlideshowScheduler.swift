...
final class SlideshowScheduler {
    ...
    /// 当前活动壁纸 ID 集合（由 RenderPipeline / UI 层维护）
    var activeWallpaperIdsProvider: (() -> Set<UUID>)?

    /// 切换壁纸回调（由调用方绑定到 RenderPipeline.setWallpaper）
    ...
    func start(context: ModelContext, settings: Settings) {
        ...
        let currentIds = activeWallpaperIdsProvider?() ?? []
    ...
    func next() {
        ...
        let currentlyShowing = activeWallpaperIdsProvider?() ?? []
