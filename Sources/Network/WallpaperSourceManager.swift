...
class WallpaperSourceManager: ObservableObject {
    ...
    enum SourceType: String, CaseIterable {
        ...
        var subtitle: String {
            switch self {
            case .wallhaven: return "Official Source"
            case .fourKWallpapers: return "Fallback Source"
            }
        }
    ...
    func recordCurrentSourceFailedAndDowngrade() -> SourceType? {
        ...
        if nextSource != activeSource {
            ...
            if !hasShownAutoSwitchToast {
                lastSwitchMessage = "⚠️ \(previousSource.displayName) unavailable, downgraded to \(nextSource.displayName)"
    ...
    private func performAutoSwitch() {
        ...
        // 只在首次自动降级时提示，后续不再重复
        if !hasShownAutoSwitchToast {
            lastSwitchMessage = "⚠️ \(previousSource.displayName) connection failed, auto-switched to \(nextSource.displayName) fallback"
            hasShownAutoSwitchToast = true
        }
